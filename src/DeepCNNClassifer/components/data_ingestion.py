...
class DataIngestion:
    ...
    def download_file(self):
        logger.info(f"{'>>>>'*5} Downloading the zip file from the url - {self.config.source_url} \
                    to file - {self.config.local_data_file} {'<<<<'*5}.")
        if not os.path.exists(self.config.local_data_file):
            ...
            logger.info(f"{'>>>>'*5} File already present in {self.config.local_data_file} {'<<<<'*5} of size\
    ...
    def unzip_and_clean(self):
        ...
        logger.info(f"{'>>>>'*5} Successfully extracted data from the zip file and saved a location - \
                    {self.config.unzip_dir} {'<<<<'*5}.")
